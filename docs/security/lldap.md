...
hook). **Prod seed users live in `platypod-sops`'s `clusters/prd/secrets.enc.yaml`**, NOT the default
`lldap.seed` in `apps/base/values/security.yaml` (prod overrides it entirely) — including the **`otel-telemetry` service
